...
{
    public partial class UserEquipmentDetails : Page
    {
        private DatabaseHelper _databaseHelper;
        private string _userName;

        public UserEquipmentDetails(string userName)
        {
            InitializeComponent();
        ...
        private void LoadData()
        {
            try
            {
                ...
                {
                    MessageBox.Show("User name is empty or null.", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                    return;
                }

                // Retrieve product details by name
                List<Product> products = _databaseHelper.GetProductDetailsbyName(_userName);

                if (products != null && products.Any())
                {
                    // Display data in the DataGrid
                    DetailsDataGrid.ItemsSource = products;
                    DetailsDataGrid.Items.Refresh();  // Refresh the DataGrid to ensure it displays the updated data
                }
                //else
                //{
        ...
        private void LoadNamesIntoComboBox()
        {
            try
            {
                List<string> names = _databaseHelper.LoadNamesbyUserName(_userName);
